...
const UserProfileDropdown = () => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            >
                {user?.employee?.photo ? (
                    <img src={user.employee.photo} alt={user?.employee?.name} className="h-full w-full object-cover" />
                ) : (
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 rounded-lg shadow-xl border border-gray-100 dark:border-gray-700 py-1 z-50">
                    <div className="px-4 py-2 border-b border-gray-100 dark:border-gray-800">
                        <p className="text-sm font-bold text-gray-900 dark:text-white truncate">{user?.employee?.name || user?.nik}</p>
